...
1. func\(\)和@func\(\)之间有什么区别？  
   第二个函数调用失败不会报错，第一个会报错

2. include\(\)和require\(\)函数的用法和区别是什么？include\_once\(\)和require\_once\(\)呢？  
...
1. preg\_replace\(\)和str\_ireplace\(\)两个函数在使用上有什么不同？preg\_split\(\)和split\(\)函数如何使用？
